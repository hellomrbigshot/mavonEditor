...
export default {
  ...
  data () {
    return {
      ...
      s_external_link: {
        ...
        hljs_js: function () {
          return '../resources/highlight.js-9.12.0/highlight.min.js';
        },
        hljs_lang: function (lang) {
          return '../resources/highlight.js-9.12.0/languages/' + lang + '.min.js';
        },
        hljs_css: function (css) {
          if (hljsCss[css]) {
            return '../resources/highlight.js-9.12.0/styles/' + css + '.min.css';
          }
          return '';
        },
        katex_js: function () {
          return '../resources/katex/katex.min.js';
        },
        katex_css: function () {
          return '../resources/katex/katex.min.css';
        }
      },
      p_external_link: {}
    };
  ...
  mounted () {
    ...
    this.loadExternalLink('katex_js', 'js', async function () {
      // await $vm.iRender(true);
    })
    this.loadExternalLink('hljs_js', 'js', async function () {
      // await $vm.iRender(true);
    })
  ...
  methods: {
    ...
    async toolbar_right_click (_type) {
      if ((['preview', 'navigation'].includes(_type) && !this.s_preview_switch) || _type === 'read') { // 切换到预览状态或者是沉浸式阅读，调用 render
        await this.iRender()
      }
    ...
    async iRender (toggleChange) {
      console.log('render')
      var $vm = this;
      let temp = ''
      if (this.replaceRules) {
        temp = await this.replaceRules($vm.d_value)
      }
  ...
  watch: {
    d_value: async function (val) {
      // v-model 语法糖
      this.$emit('input', val)
      if (this.s_preview_switch && val.trim()) { // 如果是预览状态，值变了，直接 render 一次
        this.iRender()
      }
    },
